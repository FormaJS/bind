...
import { flattenMoldErrorsToRHF, transformMoldErrorsToFormik, ValidationError } from '../src/utils.js';
...
describe('flattenMoldErrorsToRHF', () => {
  ...
  it('handles nested object errors', () => {
    ...
    const result = flattenMoldErrorsToRHF(errors);
    expect(result).toEqual({
      'user.profile.email': { type: 'validateEmailFormat', message: 'Invalid email' }
    });
  });
  ...
  it('handles array of objects with nested errors', () => {
    ...
    const result = flattenMoldErrorsToRHF(errors);
    expect(result).toEqual({
      'users.items.0.name': { type: 'validateLengthMin', message: 'Name too short' },
      'users.items.2.email': { type: 'validateEmailFormat', message: 'Invalid email' }
    });
  });
  ...
  it('handles ObjectSchema wrapper in arrays', () => {
    ...
    const result = flattenMoldErrorsToRHF(errors);
    expect(result).toEqual({
      'users.items.0.name': { type: 'validateLengthMin', message: 'Name required' }
    });
  });
...
describe('transformMoldErrorsToFormik', () => {
  ...
  it('handles array with nested objects in loop', () => {
    const errors = {
      items: [
        { rule: 'validateRequired', message: 'Required' },
        { nested: { field: [{ rule: 'validateLengthMin', message: 'Too short' }] } }
      ]
    };
    const result = flattenMoldErrorsToRHF(errors);
    expect(result).toEqual({
  ...
  it('covers array loop with non-error objects in walk function', () => {
    const errors = {
      list: [
        { someProp: 'value' },
        { anotherProp: { nested: [{ rule: 'validateMin', message: 'Too small' }] } }
      ]
    };
    const result = flattenMoldErrorsToRHF(errors);
    expect(result).toEqual({
      'list.1.anotherProp.nested': { type: 'validateMin', message: 'Too small' }
    });
  });
  ...
  it('covers for loop in walk function with mixed array elements', () => {
    const errors = {
      mixedArray: [
        ...
        { nestedObj: { field: [{ rule: 'nestedError', message: 'Nested error' }] } },
  ...
  it('covers complex array processing in walk function', () => {
    const errors = {
      complex: [
        { metadata: 'info' },
        { nested: { field: [{ rule: 'nestedRule', message: 'Nested message' }] } },
        { rule: 'directRule', message: 'Direct message' }
      ]
    };
    const result = flattenMoldErrorsToRHF(errors);
    expect(result).toEqual({
      'complex.1.nested.field': { type: 'nestedRule', message: 'Nested message' },
      ...
      company: {
        employees: {
          items: {
            ...
            2: [{
              personal: {
                phone: [{ rule: 'validateMobileNumber', message: 'Invalid phone' }]
              }
            }]
          }
        }
